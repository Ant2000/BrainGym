...
namespace GPIOHandler
{
    ...
    void IRAM_ATTR selectInterrptHandler()
    {
        gpio_num_t pin = SELECT_BUTTON_PIN;
        xQueueSendFromISR(interruptQueue, &pin, 0);
    }

    void IRAM_ATTR cycleInterrptHandler()
    {
    ...
    void setupGPIO()
    {
        ...
        attachInterrupt(SELECT_BUTTON_PIN, selectInterrptHandler, FALLING);
        attachInterrupt(CYCLE_BUTTON_PIN, cycleInterrptHandler, FALLING);
        ...
        while(1)
        {
            if(xQueueReceive(interruptQueue, &pin, portMAX_DELAY) == pdTRUE)
            {
                ...
                if(pin == SELECT_BUTTON_PIN)
                {
                    ...
                    attachInterrupt(pin, selectInterrptHandler, FALLING);
                }
                else if(pin == CYCLE_BUTTON_PIN)
                {
                    ESP_LOGI(GPIO_TAG, "Cycle button pressed");
                    xQueueSend(LCD::LCDQueue, &temp, 0);
                    attachInterrupt(pin, cycleInterrptHandler, FALLING);
                }
